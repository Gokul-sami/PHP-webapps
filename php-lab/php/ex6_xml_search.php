...
<?php
    ...
    if (isset($_GET['song_name']) && !empty(trim($_GET['song_name']))) {
        ...
        $searchQuery = ''; // Initialize to avoid errors
    }

    // Load the XML file
    $xml = simplexml_load_file('ex6_xml.xml') or die("<p>Error: Cannot load the XML file.</p>");

    // Initialize an array to store matching songs
    $matchingSongs = [];

    // Iterate through the XML to find matching songs
    foreach ($xml->song as $song) {
        // Fallback for missing XML nodes
        $name = $song->name ?? 'Unknown Name';
        $details = $song->detail ?? 'No details available';
        $link = $song->link ?? '#'; // Use a fallback link

        // Check if the song name or artist matches the search query (case-insensitive)
    ...
    // Check if any matching songs were found
